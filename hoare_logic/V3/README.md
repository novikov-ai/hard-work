...
И до Go 1.11 это работало, но с Go 1.11 перестало, так как перестал проставляться Content-Type. — Наглядный пример, когда завязались на реализацию, а не спецификацию.
...
В stdlib Go sort.Sort не гарантировал порядок равных элементов, но многие завязались на это поведение. 

В 1.19 quicksort заменилась на pdqsort и порядок стал другим, тесты, которые были на это завязаны, упали.
...
Многие используют Prometheus как библиотеку, хотя документация к сервису явно говорит о том, что это не библиотека и стабильность API не гарантируется. 

Когда Prometheus перешел на go-модули и начал менять API, то потребители начали массово ломаться, так как зависели от реализации.
